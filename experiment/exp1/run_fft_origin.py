...
from features import FeatureManager, FFTConfig
...
def run_experiment():
    print("\n" + "="*60)
    print("EXPERIMENT: FFT (Origin) - Raw vs Scaled")
    print("="*60)
    ...
    print("\n--- Step 1: Extracting FFT Features ---")
    fft_conf = FFTConfig(
        name="fft_origin",
        sample_rate=128000,
        n_fft=2048,
    ...
    fft_manager = FeatureManager(fft_conf)
    fft_data = fft_manager.get_data(loader)
    ...
    models_config = [
        (KNNConfig, {
            "name": "knn", "n_neighbors": 5, "n_jobs": 6
        }),
        (SVMConfig, {
            "name": "svm", "C": 100, "kernel": "rbf", "gamma": 0.1, 
            "probability": True, "random_state": 42
        }),
        (RFConfig, {
            "name": "rf", "n_estimators": 100, "criterion": "gini", 
            "n_jobs": 6, "random_state": 42
        }),
        (ETConfig, {
            "name": "et", "n_estimators": 200, "criterion": "gini", 
            "max_depth": 30, "n_jobs": 6, "random_state": 42
        })
    ]
    ...
    csv_path = os.path.join(os.path.dirname(__file__), "fft_origin_results.csv")
    ...
    for pipe_name, pipe_conf in pipelines.items():
        ...
        processed = processor.process_data({'fft': fft_data})
        ...
        for config_cls, params in models_config:
            ...
            save_name = f"fft_origin_{pipe_name.lower()}_{model_conf.name}_{timestamp}.joblib"
