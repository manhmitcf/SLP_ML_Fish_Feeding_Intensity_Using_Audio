...
def run_experiment():
    print("\n" + "="*60)
    print("EXPERIMENT: Fusion (FFT + MFCC + STFT) with Individual PCA")
    print("="*60)
    ...
    csv_path = os.path.join(os.path.dirname(__file__), "fusion_pca_results.csv")
    ...
    for config_cls, params in models_config:
        ...
        upstream_info = {
            "description": "Fusion of individual PCA components",
            "components": {
        ...
        dummy_config_path = f"features_cache/processed/fusion_pca_{timestamp}.json"
        import json
        ...
        save_name = f"fusion_pca_{model_conf.name}_{timestamp}.joblib"
        manager.save(save_name)
        
        row = {
            "Timestamp": timestamp,
            "Feature": "Fusion_PCA(FFT+MFCC+STFT)",
