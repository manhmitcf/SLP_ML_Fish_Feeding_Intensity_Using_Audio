...
from features import FeatureManager, FFTSConfig
...
def run_experiment():
    print("\n" + "="*60)
    print("EXPERIMENT: FFTs (Manual) - Raw vs Scaled")
    print("="*60)
    ...
    # 2. Extract FFTs Features
    print("\n--- Step 1: Extracting FFTs Features ---")
    ...
    csv_path = os.path.join(os.path.dirname(__file__), "ffts_results.csv")
    ...
    for pipe_name, pipe_conf in pipelines.items():
        ...
        processed = processor.process_data({'ffts': ffts_data})
        ...
        for config_cls, params in models_config:
            ...
            save_name = f"ffts_{pipe_name.lower()}_{model_conf.name}_{timestamp}.joblib"
