...
from features import FeatureManager, MFCCsConfig
...
def run_experiment():
    print("\n" + "="*60)
    print("EXPERIMENT: MFCCs (Pre-emphasis) - Raw vs Scaled")
    print("="*60)
    ...
    # 2. Extract MFCCs Features
    print("\n--- Step 1: Extracting MFCCs Features ---")
    ...
    mfccs_conf = MFCCsConfig(
        ...
        n_fft=2048,
        hop_length=512,
        apply_log=False # Your notebook code for MFCCs does not apply log
    )
    ...
    csv_path = os.path.join(os.path.dirname(__file__), "mfccs_results.csv")
    ...
    for pipe_name, pipe_conf in pipelines.items():
        ...
        processed = processor.process_data({'mfccs': mfccs_data})
        ...
        for config_cls, params in models_config:
            ...
            save_name = f"mfccs_{pipe_name.lower()}_{model_conf.name}_{timestamp}.joblib"
