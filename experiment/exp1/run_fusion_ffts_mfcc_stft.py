...
def run_experiment():
    print("\n" + "="*60)
    print("EXPERIMENT: Fusion (FFTs + MFCC + STFT) - Raw vs Scaled")
    print("="*60)
    ...
    # MFCC (Librosa)
    mfcc_conf = MFCCConfig(name="mfcc", sample_rate=128000, n_mfcc=13, n_fft=2048, hop_length=512)
    ...
    # A. Raw (No Scaling)
    X_train_raw = np.hstack((ffts_data['X_train'], mfcc_data['X_train'], stft_data['X_train']))
    X_test_raw = np.hstack((ffts_data['X_test'], mfcc_data['X_test'], stft_data['X_test']))
    ...
    X_train_scaled = np.hstack((ffts_scaled['X_train'], mfcc_scaled['X_train'], stft_scaled['X_train']))
    X_test_scaled = np.hstack((ffts_scaled['X_test'], mfcc_scaled['X_test'], stft_scaled['X_test']))
    ...
    csv_path = os.path.join(os.path.dirname(__file__), "fusion_ffts_mfcc_stft_results.csv")
    ...
    for data_name, (X_tr, X_te) in datasets.items():
        ...
        upstream_info = {
            "description": f"Fusion FFTs+MFCC+STFT ({data_name})",
            "components": [ffts_data['config_hash'], mfcc_data['config_hash'], stft_data['config_hash']],
            "scaling": "Separate" if data_name == "Scaled" else "None"
        }
        dummy_config_path = f"features_cache/processed/fusion_ffts_mfcc_stft_{data_name.lower()}_{timestamp}.json"
        import json
        ...
        for config_cls, params in models_config:
            ...
            save_name = f"fusion_ffts_mfcc_stft_{data_name.lower()}_{model_conf.name}_{timestamp}.joblib"
            manager.save(save_name)
            
            row = {
                "Timestamp": timestamp,
                "Feature": "Fusion_FFTs_MFCC_STFT",
                "Pipeline": data_name,
